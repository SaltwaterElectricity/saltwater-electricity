  ...
  if (devicesLoading) {
    return (
      <div className="min-h-screen bg-background p-8 flex flex-col items-center justify-center">
        ...
        <p className="text-label-sm font-semibold text-outline uppercase tracking-widest mt-4 animate-pulse font-['Inter']">
    ...
    return (
      <div className="min-h-screen bg-background p-8 flex items-center justify-center text-center">
        <div className="max-w-md p-12 glass-panel shadow-xl animate-in fade-in zoom-in-95 duration-500">
          ...
          <h2 className="text-h2 font-['Space_Grotesk'] font-bold text-primary tracking-tight italic">
            No Active Node
          </h2>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
        ...
        <div className="h-full flex flex-col justify-start">
          ...
          <div className="mt-6 p-6 bg-primary rounded-[20px] text-white shadow-xl shadow-primary/20 group hover:scale-[1.02] transition-transform duration-500">
            <p className="text-label-sm font-bold uppercase tracking-widest text-primary-fixed mb-2 font-['Inter']">
              Facility Guard
            </p>
            <p className="text-body-md font-bold leading-tight font-['Inter']">
              Your water quality is within the{" "}
              <span className="underline decoration-secondary-container">excellent</span> range.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <main className="lg:col-span-2 glass-panel p-8 shadow-sm flex flex-col min-h-[500px] transition-all hover:bg-white/80">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-label-sm font-bold uppercase tracking-widest text-on-surface leading-none font-['Inter']">
                Salinity Performance
              </h3>
              <p className="text-label-sm font-semibold text-outline uppercase tracking-widest mt-2 font-['Inter']">
                7-Day Trend Analysis
              </p>
            </div>
            <div className="flex gap-2">
              <span className="text-label-sm font-bold px-3 py-1.5 bg-primary/5 text-primary rounded-full border border-primary/10 uppercase tracking-widest font-['Inter']">
                Historical View
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 min-h-[300px]">
            {logsLoading ? (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-label-sm font-semibold text-outline uppercase tracking-[0.2em] font-['Inter']">
                  Syncing Archive...
                </p>
              </div>
            ) : (
              <DeviceAnalyticsChart data={chartData} metricConfig={METRIC_CONFIG[METRICS.TDS]} />
            )}
          </div>
        </main>
        ...
        <aside className="bg-primary-container rounded-[20px] p-8 text-white shadow-2xl overflow-hidden relative group h-full">
          ...
          <div className="space-y-8 relative z-10 overflow-y-auto max-h-[400px] custom-scrollbar-hide">
            {logs.slice(0, 5).map((log) => (
              <div
                key={log.id}
                className="relative pl-6 border-l border-white/10 animate-in fade-in slide-in-from-left-4 duration-500"
              >
                ...
                <p className="text-label-sm text-white/80 font-medium font-['Inter']">
                  System recorded{" "}
                  <span className="text-secondary-container font-bold">{log.tds_ppm} PPM</span> with
                  node voltage at{" "}
                  <span className="text-tertiary-fixed font-bold">{log.voltage}V</span>.
                </p>
              </div>
            ))}
            ...
            {logs.length === 0 && !logsLoading && (
              <div className="py-12 text-center">
                <p className="text-label-sm font-bold text-white/40 uppercase tracking-widest font-['Inter']">
                  No Stream Activity
                </p>
              </div>
            )}
