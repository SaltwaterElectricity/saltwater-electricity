...
const AdminRealTimeMonitor = () => {
  ...
  const hydratedDevices = useMemo(() => {
    ...
    return devices.map(device => {
      ...
      // Determine status based on telemetry thresholds
      let status = "Offline";
      if (isOnline) {
        if (tel.tds_ppm > 800 || tel.voltage > 250 || tel.voltage < 210) {
          status = "Warning";
        } else {
          status = "Online";
        }
  ...
  const filteredDevices = useMemo(() => {
    return hydratedDevices.filter(d => {
      ...
      const matchesLocation = locationFilter === "All Locations" || d.residentLocation === locationFilter;
  ...
  const stats = useMemo(() => {
    ...
    const totalSalinity = hydratedDevices.reduce((acc, d) => acc + (d.telemetry?.tds_ppm || 0), 0);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] gap-6 animate-pulse">
        ...
        <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">Synchronizing Grid...</p>
      </div>
    );
  }
      ...
      <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar pb-10">
        ...
        {filteredDevices.length === 0 && (
          <div className="py-20 text-center bg-white rounded-[24px] border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">No nodes match the filter spectrum</p>
          </div>
        )}
      </div>
