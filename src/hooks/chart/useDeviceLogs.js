...
// import { subscribeToDeviceLogs } from '../../services/firebaseService'; // Broken import
...
export const useDeviceLogs = (mac, limit = 50, startDate = null) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Since subscribeToDeviceLogs is missing, we log a warning but use structured error
      console.warn("[useDeviceLogs] subscribeToDeviceLogs service missing. Check your imports.");
      setError(new appError("Monitoring service integration missing.", false, "service/missing"));
      setLoading(false);
      ...
      unsubscribe = subscribeToDeviceLogs(
        ...
        (err) => {
          ...
          setError(new appError("Failed to sync with device. Check permissions.", true, err.code || "db/sync-failed"));
          setLoading(false);
        }
      );
      */
      ...
      setError(new appError("Connection setup failed.", false, "db/setup-failed"));
      setLoading(false);
    }
