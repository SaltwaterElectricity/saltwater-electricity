...
/**
 * TotalDevicesCard Component (Resident Dashboard Exclusive)
 * Mirrors the specific design from user-dashboard.html
 */
const TotalDevicesCard = memo(({ value, status = "Active" }) => {
  return (
    <div className="bg-white rounded-2xl p-6 flex items-center shadow-[0_12px_32px_-4px_rgba(10,46,255,0.04)] border border-[#e4e7ec80] min-w-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="flex items-center gap-1.5 text-green-600 text-[12px] font-semibold mb-2">
      ...
      <div className="flex-shrink-0 flex items-end gap-1 h-12 ml-4 opacity-60 md:hidden xl:flex">
        <div className="w-1.5 bg-primary/30 h-1/4 rounded-full" />
        <div className="w-1.5 bg-primary/40 h-1/2 rounded-full" />
        <div className="w-1.5 bg-primary/50 h-3/4 rounded-full" />
...
TotalDevicesCard.displayName = "TotalDevicesCard";
