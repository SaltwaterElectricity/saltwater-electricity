...
const HistoricalFilterBar = ({ 
  ...
  const deviceLabel = selectedDeviceId === "all" ? "All Device Readings" : (selectedDevice?.device_name || selectedDeviceId);
    ...
    <section className="px-xl mt-lg">
      <div className="bg-white p-md rounded-xl border border-outline-variant flex items-center gap-md flex-wrap">
        ...
        <div className="flex-1 min-w-[200px] relative">
          <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline text-primary-container">search</span>
          <input 
        ...
        <div className="relative">
          ...
          >
            ...
            {deviceLabel}
            <span className="material-symbols-outlined text-outline text-primary-container">keyboard_arrow_down</span>
          </button>
          ...
          {isDeviceMenuOpen && (
            <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-outline-variant rounded-lg shadow-lg z-50 overflow-hidden">
              <div className="py-1">
                <button 
                  onClick={() => { onDeviceChange("all"); setIsDeviceMenuOpen(false); }}
                ...
                {devices.map(dev => (
                  <button 
                    key={dev.device_id}
                    onClick={() => { onDeviceChange(dev.device_id); setIsDeviceMenuOpen(false); }}
                    className="w-full text-left px-4 py-2 text-body-sm hover:bg-surface-container-low transition-colors"
                  >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="flex gap-1">
                        <span className="material-symbols-outlined text-outline cursor-pointer text-[18px]">chevron_left</span>
                        <span className="material-symbols-outlined text-outline cursor-pointer text-[18px]">chevron_right</span>
                      </div>
                    </div>
        ...
        >
          <span className="material-symbols-outlined text-[20px] text-primary-container">filter_list_off</span>
          Clear Filters
        </button>
      </div>
    </section>
