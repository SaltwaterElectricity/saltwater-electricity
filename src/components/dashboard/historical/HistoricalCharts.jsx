...
const HistoricalCharts = ({ logs = [], loading = false }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(groups).map(([key, data]) => ({
      name: granularity === "WEEKLY" ? key : new Date(data.ts).toLocaleDateString([], { month: "short" }),
  ...
  const distributionData = useMemo(() => {
    ...
    return [
      { name: "Voltage", value: counts.voltage, color: "#003594", percent: ((counts.voltage / totalAll) * 100).toFixed(1) },
      { name: "Salinity", value: counts.salinity, color: "#0891b2", percent: ((counts.salinity / totalAll) * 100).toFixed(1) },
      { name: "Current", value: counts.current, color: "#ea580c", percent: ((counts.current / totalAll) * 100).toFixed(1) },
    ];
  }, [logs]);
  ...
  const powerStats = useMemo(() => {
    ...
    return {
      ...
      sensors: { val: (avgSensors / 1000).toFixed(2) + " A", pct: Math.round((avgSensors / totalMa) * 100) },
    };
  }, [logs]);
    ...
    <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-8 bg-white p-3.5 rounded-2xl border border-outline-variant shadow-sm flex flex-col min-h-[230px] hover:shadow-xl hover:shadow-primary/5 transition-all duration-500">
        ...
        <div className="flex items-center gap-5 mb-2.5 border-b border-outline-variant/5 pb-2.5">
          <SeriesToggle label="Current" color="#ea580c" active={visible.c} onClick={() => toggleVisibility('c')} />
          <SeriesToggle label="Voltage" color="#003594" active={visible.v} onClick={() => toggleVisibility('v')} />
          <SeriesToggle label="Salinity" color="#0891b2" active={visible.s} onClick={() => toggleVisibility('s')} />
        </div>
        ...
        <div className="flex-1 w-full min-h-[90px]">
          {loading ? (
            <div className="w-full h-full flex items-center justify-center bg-surface-container-lowest animate-pulse rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-widest text-outline">Syncing...</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart key={`${granularity}-${Object.values(visible).join('')}`} data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: -30 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#c3c6d6" opacity={0.15} />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "#565e74", fontSize: 8, fontWeight: 800 }} dy={3} />
                <YAxis hide domain={["auto", "auto"]} />
                <Tooltip
                  cursor={{ stroke: '#f1f5f9', strokeWidth: 8 }}
                  contentStyle={{ borderRadius: "8px", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(0, 0, 0, 0.05)", padding: "6px 8px" }}
                  labelStyle={{ fontWeight: 900, marginBottom: "1px", color: "#1e293b", fontSize: "10px", textTransform: 'uppercase' }}
                />
                {visible.v && (
                  <Bar dataKey="v" fill="#003594" opacity={0.25} radius={[2, 2, 0, 0]} barSize={8} isAnimationActive={true} animationBegin={0} animationDuration={1200} />
                )}
                {visible.c && (
                  <Bar dataKey="c" fill="#ea580c" opacity={0.25} radius={[2, 2, 0, 0]} barSize={8} isAnimationActive={true} animationBegin={0} animationDuration={1200} />
                )}
                {visible.s && (
                  <Line type="monotone" dataKey="s" stroke="#0891b2" strokeWidth={2} dot={{ r: 1.5, fill: "#fff", strokeWidth: 1.5 }} activeDot={{ r: 3.5, strokeWidth: 0 }} isAnimationActive={true} animationBegin={0} animationDuration={1200} />
                )}
              </ComposedChart>
            </ResponsiveContainer>
          )}
        ...
        <div className="bg-white rounded-2xl border border-outline-variant shadow-sm p-3.5 border-primary h-full hover:shadow-xl hover:shadow-primary/5 transition-all duration-500">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="relative w-full aspect-square max-w-[80px] mx-auto">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={distributionData} innerRadius="70%" outerRadius="95%" paddingAngle={3} dataKey="value" stroke="none" isAnimationActive={true} animationBegin={0} animationDuration={1200}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <p className="text-[10px] font-black text-primary tracking-tight leading-none">{totalReadings}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-0.5">
              {distributionData.map((item) => (
                <div key={item.name} className="flex items-center gap-2 px-2 py-1 rounded-lg bg-surface-container-low/20 border border-outline-variant/5">
                  <span className="w-1.5 h-1.5 rounded-full flex-none" style={{ backgroundColor: item.color }} />
          ...
          <div className="mt-3.5 pt-3.5 border-t border-outline-variant/10">
            ...
            <div className="space-y-2.5">
              <UsageBar icon="developer_board" label="System (ESP32)" value={powerStats.esp.val} percent={powerStats.esp.pct} color="#9333ea" />
              <UsageBar icon="bolt" label="External Load" value={powerStats.load.val} percent={powerStats.load.pct} color="#ec4899" />
              <UsageBar icon="sensors" label="Sensor Array" value={powerStats.sensors.val} percent={powerStats.sensors.pct} color="#0ea5e9" />
            </div>
          </div>
        </div>
      </div>
...
const SeriesToggle = ({ label, color, active, onClick }) => (
  ...
  >
    <div className="w-2 h-2 rounded-full border shadow-sm" style={{ backgroundColor: color, borderColor: color }} />
  ...
  <div className="flex flex-col gap-1.5 animate-in fade-in slide-in-from-left-2 duration-700">
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-black text-on-surface uppercase tracking-tighter">{label}</span>
      </div>
    ...
    <div className="relative w-full bg-surface-container-low rounded-full overflow-hidden flex items-center h-2.5 border border-outline-variant/5 shadow-inner">
      ...
      >
        <span className="text-white font-black absolute right-2 text-[7px] drop-shadow-sm">{percent}%</span>
      </div>
    </div>
  </div>
);
