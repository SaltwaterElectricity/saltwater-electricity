...
import { X, User, Calendar, Phone, Server, MapPin, Activity, CheckCircle2, AlertTriangle, WifiOff, Bolt } from "lucide-react";
...
const MiniGraphSection = ({ label, value, unit, color, data }) => (
  <div className="space-y-4">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{label} ({unit})</p>
      ...
      {data.map((h, i) => (
        <div 
          ...
          className={cn(
            ...
            color === 'blue' ? "bg-primary/40 hover:bg-primary" : "bg-purple-400 hover:bg-purple-600"
          )} 
          style={{ height: `${h}%` }} 
        />
      ))}
      <div className="absolute top-2 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
         <div className="bg-slate-900 text-white text-[8px] font-black px-2 py-1 rounded shadow-lg uppercase tracking-tighter">Live Pulse</div>
      </div>
    </div>
  </div>
);
        ...
        <div className="h-full flex flex-col min-w-[450px]">
          ...
          <div className="sticky top-0 bg-white/95 backdrop-blur-xl z-10 p-8 border-b border-slate-50">
            <div className="flex justify-end mb-6">
              <button onClick={onClose} className="p-2 hover:bg-slate-50 rounded-full transition-colors border border-slate-100 shadow-sm active:scale-90">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-xl font-bold text-slate-900 tracking-tight">{device.device_id}</h2>
                  <span className={cn(
                    "px-2.5 py-0.5 text-[9px] font-bold rounded uppercase tracking-wider border shadow-sm",
                    device.status === "Online" ? "bg-green-50 text-green-600 border-green-100" :
                    device.status === "Warning" ? "bg-orange-50 text-orange-600 border-orange-100" :
                    "bg-slate-50 text-slate-400 border-slate-100"
                  )}>
                    {device.status}
                  </span>
                </div>
                <p className="text-sm font-semibold text-slate-500 italic mt-1">{device.residentName}</p>
                <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{device.residentEmail}</p>
              </div>
            </div>
            ...
            {activeTab === 'alerts' && (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                 <div className="flex items-center justify-between mb-4">
                    <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Node Incident Logs</h3>
                    <button className="text-[10px] font-bold text-primary hover:underline">View Archive</button>
                 </div>
                 <AlertRow icon={CheckCircle2} title="Voltage Stabilization" desc="Telemetry reported return to nominal voltage spectrum." time="Just Now" status="success" />
                 <AlertRow icon={AlertTriangle} title="High Load" desc="Ion concentration threshold breach detected." time="10m ago" status="warning" />
                 <AlertRow icon={WifiOff} title="Signal Interrupted" desc="Temporary telemetry blackout reported." time="25m ago" status="muted" />
                 <AlertRow icon={Bolt} title="Primary Cell Reset" desc="Hardware initiated automated cell membrane restart." time="1h ago" status="muted" />
              </div>
            )}
          </div>
        </div>
