...
import { Calendar, Clock, ChevronDown, Cpu, ShieldAlert, Fingerprint, Target, Terminal } from "lucide-react";
...
const AuditLogTableRow = ({ log }) => {
  ...
  // System/Guest Flags
  const isSystemAccount = actorEmail === "system@saltwaterelectricity.internal" || log.adminName === "System";
  ...
  const displayName = fullName || 
                     ...
                     (isSystemAccount ? (log.action?.includes("DEVICE") || log.action?.includes("RELAY") ? log.targetId || "IoT System" : "System Automated Action") : null) ||
                     actorEmail;
      ...
      <tr className={cn(
        ...
        <td className="px-6 py-4">
          <span className={cn(
            ...
            (displayRole === "System" || displayRole === "Guest" || displayRole === "User") && "bg-slate-100 text-slate-600 border-slate-200"
          )}>
            {displayRole}
          </span>
        </td>
          ...
          <td colSpan="8" className="px-6 py-6 border-b border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2">Actor Identity</p>
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2">Technical Context</p>
                <DetailItem icon={Fingerprint} label="Actor UID" value={log.actorUid || "N/A"} mono />
                <DetailItem icon={Target} label="Target ID" value={log.targetId || "N/A"} mono />
                <DetailItem icon={Terminal} label="Client Node" value={log.ipAddress || "Internal System"} mono />
              </div>

              {/* Event details */}
              <div className="col-span-2 space-y-4">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2">Full Narrative</p>
                ...
                <div className="flex flex-wrap gap-4 pt-2">
                  <div className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">
                    Recorded At: <span className="text-slate-600 ml-1">{new Date(log.createdAt).toLocaleString()}</span>
                  </div>
                  <div className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">
...
const DetailItem = ({ icon: Icon, label, value, mono }) => (
  <div className="flex items-start gap-3">
    ...
    <div className="min-w-0">
      ...
      <p className={cn(
        "text-[11px] font-bold text-slate-700 truncate",
        mono && "font-mono"
      )}>{value}</p>
    </div>
  </div>
