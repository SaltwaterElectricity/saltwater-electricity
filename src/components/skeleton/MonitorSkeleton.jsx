    ...
    <div className="flex flex-col h-full relative overflow-hidden space-y-0 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {[1, 2].map((i) => (
          <div
            key={i}
            className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 flex items-center h-[108px]"
          >
            ...
            <div className="flex items-end space-x-1 h-12 ml-4 shrink-0">
              {[1, 2, 3, 4, 5, 6].map((j) => (
                <div
                  key={j}
                  className="w-1.5 h-full bg-slate-50 rounded-full animate-pulse"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 flex items-center h-[98px]"
          >
            ...
            <div className="flex items-end space-x-1 h-8 ml-2">
              {[1, 2, 3, 4].map((j) => (
                <div
                  key={j}
                  className="w-1 h-full bg-slate-50 rounded-full animate-pulse"
      ...
      <div className="space-y-4 overflow-hidden">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-white px-6 py-4 rounded-[24px] border border-gray-100 flex items-center h-[100px]"
          >
                ...
                <div key={j} className="w-40 space-y-2">
                  ...
                  <div className="flex items-end space-x-1 h-8">
                    {[1, 2, 3, 4, 5, 6].map((k) => (
                      <div
                        key={k}
                        className="flex-1 h-full bg-slate-50 rounded-sm animate-pulse"
