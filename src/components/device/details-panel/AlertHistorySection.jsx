...
export const AlertHistorySection = ({ deviceId, auditLogs }) => {
  ...
  return (
    <section
      id="section-alerts"
      className="scroll-mt-6 pb-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200"
    >
      ...
      <div className="space-y-4">
        {deviceLogs.length > 0 ? (
          deviceLogs.map((log, idx) => (
            <AlertItem key={log.id} log={log} index={idx} isMounted={isMounted} />
          ))
        ) : (
...
const AlertItem = ({ log, index, isMounted }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute left-0 top-0 w-1 h-full transition-transform duration-500 scale-y-0 group-hover:scale-y-100",
          config.color.replace("text", "bg")
        )}
      />
    </div>
