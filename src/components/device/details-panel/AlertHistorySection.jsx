...
export const AlertHistorySection = ({ deviceId, auditLogs }) => {
  ...
  return (
    <section id="section-alerts" className="scroll-mt-6">
      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-5">Alert History</h3>
  ...
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-xl border border-slate-50 hover:bg-slate-50 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] font-bold text-slate-700 truncate">{log.action?.replace(/_/g, " ")}</p>
      ...
      <div className="text-right">
        ...
        <div className={cn("w-1 h-1 rounded-full ml-auto mt-1", config.color.replace('text', 'bg'))} />
      </div>
    </div>
  );
};
