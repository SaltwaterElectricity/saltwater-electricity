...
export const HardwareUsageSection = ({ telemetry }) => {
  ...
  const components = useMemo(() => {
    ...
    return [
      { id: "esp", name: "ESP32 Controller", value: esp, icon: Cpu, color: "from-blue-500 to-cyan-500" },
      { id: "volt", name: "Voltage Sensor", value: vSensor, icon: Zap, color: "from-blue-500 to-cyan-500" },
      { id: "sal", name: "Salinity Sensor", value: sSensor, icon: Droplets, color: "from-blue-500 to-cyan-500" },
      { id: "relay", name: "Relay Driver Module", value: relay, icon: Settings, color: "from-blue-500 to-cyan-500" },
      { id: "bulb", name: "Smart Bulb", value: bulb, icon: Lightbulb, color: "from-blue-500 to-cyan-500" },
    ];
    ...
    <section id="section-components" className="scroll-mt-6">
      <div className="flex flex-col gap-0.5 mb-6">
        <div className="flex justify-between items-center">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Hardware Consumption</h3>
          <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 rounded-full border border-emerald-100">
            <div className="w-1 h-1 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[8px] font-bold text-emerald-600 uppercase tracking-widest">Live Stream</span>
          </div>
        </div>
        <p className="text-[11px] text-slate-400 font-medium italic">Real-time current draw of modules.</p>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-[28px] p-6 mb-8 shadow-sm relative overflow-hidden flex flex-col items-center border-b-primary border-b-2">
        <div className="relative w-52 h-52 flex items-center justify-center">
          ...
          <div className="text-center z-10 flex flex-col items-center">
            <span className="text-[44px] font-black font-display leading-none text-slate-900">{currentTotal}</span>
            <span className="text-[14px] font-black text-slate-400 mt-0.5 uppercase tracking-widest">mA</span>
        ...
        <div className="flex flex-col items-center mt-6 gap-2">
          ...
          <div className="flex items-center gap-1.5 text-emerald-600">
             ...
             <span className="text-[9px] font-black uppercase tracking-[0.1em]">Optimal Efficiency</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2.5">
        {components.map((comp) => (
          <div key={comp.id} className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 flex items-center gap-3 transition-all hover:bg-white hover:shadow-md hover:shadow-slate-100/50 group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[7px] font-black text-emerald-500 uppercase tracking-tighter">Active</span>
              </div>
            ...
            <div className="text-right">
              <p className="font-mono text-[11px] font-black text-slate-900">{comp.value} <span className="text-[9px] text-slate-400">mA</span></p>
            </div>
          </div>
        ))}
      </div>
