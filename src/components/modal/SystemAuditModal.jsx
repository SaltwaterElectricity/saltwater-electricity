  ...
  return (
    <ModalBackdrop>
      <div className="bg-white/90 backdrop-blur-2xl rounded-[40px] shadow-[0_30px_100px_rgba(0,0,0,0.2)] w-[95vw] max-w-5xl h-[85vh] flex flex-col overflow-hidden border border-white/40 border-t-white/60 animate-in zoom-in-95 duration-300">
        ...
        {/* MODAL HEADER */}
        <header className="p-6 md:p-8 flex items-center justify-between border-b border-slate-100 bg-white/30">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-none uppercase italic truncate">
                {deviceName || 'System'} <span className="text-blue-600">Audit</span>
              </h2>
              <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2 truncate">
                Node: {deviceId}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
          ...
          <nav className="flex md:flex-col overflow-x-auto md:overflow-y-auto border-b md:border-b-0 md:border-r border-slate-100 p-2 md:p-6 gap-2 bg-slate-50/50 custom-scrollbar-hide">
            <NavButton 
            ...
            <div className="md:pt-8 hidden md:block">
                <button className="w-full py-4 bg-slate-900 text-white rounded-xl text-[10px] font-black tracking-widest uppercase flex items-center justify-center gap-2 hover:bg-blue-600 transition-colors shadow-lg active:scale-95">
                    <Download size={14} />
                    Export
                </button>
            </div>
          ...
          {/* MAIN CONTENT AREA */}
          <main className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-white/10">
...
const NavButton = ({ active, onClick, icon: Icon, label }) => (
  <button 
    onClick={onClick}
    className={cn(
      "w-full flex flex-col md:flex-row items-center md:items-center gap-2 md:gap-3 px-2 md:px-4 py-3 rounded-xl transition-all duration-300 group",
      active 
...
const SectionHeader = ({ title, sub }) => (
    <div className="mb-6 md:mb-8">
        <h3 className="text-base md:text-lg font-black text-slate-900 tracking-tight uppercase leading-none">{title}</h3>
        <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">{sub}</p>
    </div>
);
...
const AnalyticsSection = ({ logs, loading }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fadeIn">
            ...
            <div className="h-48 md:h-64 bg-slate-50/50 rounded-3xl border border-slate-100 p-4 md:p-6">
    ...
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-100 bg-white shadow-sm">
            <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100">
                    <tr>
                        <th className="p-3 md:p-4">Time</th>
                        <th className="p-3 md:p-4 text-center">TDS</th>
                        <th className="p-3 md:p-4 text-center">Volt</th>
                        <th className="p-3 md:p-4 text-right">Relay</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                    {logs.slice(0, 15).map(log => (
                        <tr key={log.id} className="text-[10px] md:text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors">
                            <td className="p-3 md:p-4 whitespace-nowrap">{new Date(log.__normalizedTs).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</td>
                            <td className="p-3 md:p-4 text-center font-mono text-blue-600">{log.tds_ppm}</td>
                            <td className="p-3 md:p-4 text-center font-mono text-slate-400">{log.voltage}V</td>
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="p-5 md:p-6 bg-blue-50/40 rounded-3xl border border-blue-100/50 backdrop-blur-sm">
                ...
                <div className="p-5 md:p-6 bg-slate-50/50 rounded-3xl border border-slate-200/50 backdrop-blur-sm">
...
const ConsumptionSection = ({ logs }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fadeIn">
            ...
            <div className="flex flex-col items-center justify-center p-8 md:p-12 bg-slate-900 rounded-[32px] text-white shadow-2xl relative overflow-hidden group">
